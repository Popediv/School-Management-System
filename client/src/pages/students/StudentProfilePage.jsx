...
export default function StudentProfilePage() {
  ...
  const { data: ledgerData } = useQuery({
    ...
    queryFn: () => feeService.getLedger(id).then(r => r.data),
    enabled: showLedgerModal,
  });
    ...
    <div>
        ...
        <div id="fees-section" className="card mt-6" style={{ padding: 0 }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowLedgerModal(true)} className="btn btn-secondary btn-sm">
                <FileText size={14} /> Statement of Account (Ledger)
              </button>
      ...
      {showLedgerModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div className="card animate-slide-up" style={{ width: '100%', maxWidth: 850, maxHeight: '90vh', overflowY: 'auto', padding: 0 }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--bg-surface)', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: 8 }}><ClipboardCheck size={20} style={{ color: 'var(--primary)' }} /> Student Statement of Account</h3>
                <p className="text-sm text-muted mt-1">Detailed Ledger for {student?.lastName} {student?.firstName} ({student?.admissionNum})</p>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <button onClick={() => window.print()} className="btn btn-secondary btn-sm" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Printer size={15} /> Print Statement
                </button>
                <button onClick={() => setShowLedgerModal(false)} className="btn btn-secondary btn-icon"><X size={18} /></button>
              </div>
            </div>

            <div style={{ padding: 24 }}>
              {!ledgerData ? (
                <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Loading ledger data...</div>
              ) : (
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
                    <div style={{ background: 'var(--bg-body)', padding: 16, borderRadius: 8, border: '1px solid var(--border)' }}>
                      <div className="text-xs text-muted font-semibold uppercase">Total Invoiced (Debits)</div>
                      <div className="text-xl font-bold text-danger mt-1">
                        ₦{(ledgerData.ledger?.reduce((acc, curr) => acc + (curr.debit || 0), 0) || 0).toLocaleString()}
                      </div>
                    </div>
                    <div style={{ background: 'var(--bg-body)', padding: 16, borderRadius: 8, border: '1px solid var(--border)' }}>
                      <div className="text-xs text-muted font-semibold uppercase">Total Paid (Credits)</div>
                      <div className="text-xl font-bold text-success mt-1">
                        ₦{(ledgerData.ledger?.reduce((acc, curr) => acc + (curr.credit || 0), 0) || 0).toLocaleString()}
                      </div>
                    </div>
                    <div style={{ background: 'var(--bg-body)', padding: 16, borderRadius: 8, border: '1px solid var(--border)' }}>
                      <div className="text-xs text-muted font-semibold uppercase">Net Outstanding Balance</div>
                      <div className={`text-xl font-bold mt-1 ${ledgerData.currentBalance > 0 ? 'text-danger' : 'text-success'}`}>
                        ₦{(ledgerData.currentBalance || 0).toLocaleString()}
                      </div>
                    </div>
                  </div>
                      ...
                      <tbody>
                        {ledgerData.ledger?.map(entry => (
                          <tr key={entry.id}>
                            ...
                            <td><code style={{ background: 'var(--bg-body)', padding: '2px 6px', borderRadius: 4, fontSize: '0.8rem' }}>{entry.reference}</code></td>
                            <td>{entry.description}</td>
                            <td style={{ textAlign: 'right', color: 'var(--danger)', fontWeight: entry.debit > 0 ? '600' : 'normal' }}>{entry.debit > 0 ? `₦${entry.debit.toLocaleString()}` : '—'}</td>
                            <td style={{ textAlign: 'right', color: 'var(--success)', fontWeight: entry.credit > 0 ? '600' : 'normal' }}>{entry.credit > 0 ? `₦${entry.credit.toLocaleString()}` : '—'}</td>
                            <td style={{ textAlign: 'right', fontWeight: 'bold' }}>₦{entry.balance?.toLocaleString()}</td>
                          </tr>
                        ))}
                        {(!ledgerData.ledger || ledgerData.ledger.length === 0) && (
                          <tr><td colSpan="6" className="text-center text-muted py-8">No transaction ledger entries found for this student.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
