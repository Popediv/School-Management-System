...
import { studentService, classService, attendanceService, digitalPersonaService } from '../../services';
import { Upload, UserPlus, Camera, X, Fingerprint, CheckCircle, Loader } from 'lucide-react';
...
export default function StudentRegisterPage() {
  ...
  const [fpStatus, setFpStatus] = useState(null); // null | 'scanning' | 'captured' | 'offline'
  ...
  const handleFingerprintCapture = async () => {
    ...
    try {
      const dpResult = await digitalPersonaService.captureFingerprint();
      if (dpResult.success && dpResult.template) {
        setFingerprintTemplate(dpResult.template.trim());
        setFpStatus('captured');
        toast.success('Fingerprint captured from DigitalPersona reader!');
      } else {
        setFpStatus('offline');
        toast.warning('DigitalPersona reader not detected on local ports. Please check USB connection and driver.');
      }
    } catch (err) {
      console.error(err);
      setFpStatus('offline');
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (fingerprintTemplate) fd.append('fingerprintTemplate', fingerprintTemplate);

      const res = await studentService.create(fd);

      // If fingerprint was captured, also save it via the dedicated endpoint
      if (fingerprintTemplate && res.data.id) {
        try {
          await attendanceService.saveFingerprint({ studentId: res.data.id, fingerprintTemplate });
        } catch { /* non-critical, already in FormData */ }
      }
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid-2">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {false && (
              <div className="card" style={{ borderColor: fpStatus === 'captured' ? 'var(--success)' : 'var(--border)' }}>
                <h3 className="mb-2" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Fingerprint size={20} style={{ color: fpStatus === 'captured' ? 'var(--success)' : 'var(--primary)' }} />
                  Fingerprint Enrollment
                  {fpStatus === 'captured' && <span className="badge badge-success" style={{ marginLeft: 'auto', fontSize: '0.7rem' }}>Captured ✓</span>}
                </h3>
                <p className="text-xs text-muted" style={{ marginBottom: 12 }}>
                  {fpStatus === 'captured'
                    ? 'Fingerprint captured and ready to save with student record.'
                    : 'Enroll the student\'s fingerprint using your connected DigitalPersona U.are.U 4500 USB reader.'
                  }
                </p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  <button
                    type="button"
                    className={`btn ${fpStatus === 'scanning' ? 'btn-secondary' : fpStatus === 'captured' ? 'btn-secondary' : 'btn-primary'} btn-sm`}
                    disabled={fpStatus === 'scanning'}
                    onClick={handleFingerprintCapture}
                  >
                    {fpStatus === 'scanning'
                      ? <><Loader size={14} className="animate-spin" /> Scanning Reader...</>
                      : fpStatus === 'captured'
                        ? <><Fingerprint size={14} /> Re-Capture Fingerprint</>
                  ...
                  >
                    {showManualInput ? 'Hide manual entry' : 'Manual key input'}
                  </button>
                </div>

                {showManualInput && (
                  <div style={{ marginTop: 12, padding: 10, background: 'var(--bg-elevated)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
                    <label className="text-xs text-muted" style={{ display: 'block', marginBottom: 4 }}>
                      Manual Fingerprint Identifier / Test Key:
                    </label>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <input
                        type="text"
                        className="form-input text-xs"
                        placeholder="e.g. PCI-260100"
                        value={fingerprintTemplate || ''}
                        onChange={(e) => {
                          setFingerprintTemplate(e.target.value);
                          if (e.target.value.trim()) setFpStatus('captured');
                ...
                {fpStatus === 'captured' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--success)', fontSize: '0.82rem', marginTop: 10 }}>
                    <CheckCircle size={16} /> Fingerprint ready — will be saved on submit.
                  </div>
                )}
                {fpStatus === 'offline' && (
                  <div style={{ color: 'var(--warning)', fontSize: '0.82rem', marginTop: 8 }}>
                    Scanner device not detected on USB / driver port. Check reader connection or use manual entry.
                  </div>
                )}
              </div>
            )}
